...
function initializeDOMElements() {
  ...
  console.log('DOM elements: searchToggle=', !!document.getElementById('searchToggle'),
              ...
              'viewPanelToggle=', !!document.getElementById('viewPanelToggle'));
}
...
function attachQuickActionListeners() {
  ...
  const viewPanelToggle = document.getElementById('viewPanelToggle');
  let viewPanelListenerAttached = false;
  if (viewPanelToggle) {
    try {
      viewPanelToggle.addEventListener('click', toggleViewPanel);
      viewPanelListenerAttached = true;
    } catch (e) {
      console.warn('Unable to attach view panel toggle listener', e);
    }
  } else {
    console.warn('No #viewPanelToggle element found');
  }
  ...
  if (!viewPanelListenerAttached) {
    document.addEventListener('click', (e) => {
      const t = e.target.closest('#viewPanelToggle');
      if (t) toggleViewPanel(e);
    });
  }
}
...
function toggleViewPanel(evt) {
  evt.stopPropagation();
  const panel = document.getElementById('viewModeContainer');
  const btn = document.getElementById('viewPanelToggle');
  togglePanel(panel, btn);
}

function closePanelsOnOutsideClick(e) {
  // If click is not within panel or corresponding toggle buttons, hide all panels
  const panels = ['searchSection', 'advancedFilters', 'viewModeContainer'];
  panels.forEach(id => {
    const panel = document.getElementById(id);
    const toggleBtn = document.getElementById(id === 'searchSection' ? 'searchToggle' : id === 'advancedFilters' ? 'filtersToggle' : 'viewPanelToggle');
...
function handleKeyboardShortcuts(e) {
  ...
  if (e.key === 'Escape') {
    ...
    ['searchSection','advancedFilters','viewModeContainer'].forEach(id => {
      const panel = document.getElementById(id);
      const btn = document.getElementById(id === 'searchSection' ? 'searchToggle' : id === 'advancedFilters' ? 'filtersToggle' : 'viewPanelToggle');
      if (panel && !panel.classList.contains('hidden')) {
        panel.classList.add('hidden');
        panel.setAttribute('aria-hidden', 'true');
