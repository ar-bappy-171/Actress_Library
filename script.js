...
function initializeApp() {
  ...
  // Ensure event listeners for actionable header icons are attached even
  // if some DOM nodes are dynamically replaced or added later.
  try {
    attachQuickActionListeners();
  } catch (err) {
    console.warn('Failed to re-attach quick action listeners:', err);
  }
  ...
  updateStats();
  // Ensure view-mode container is hidden after returning to list
  const v = document.getElementById('viewModeContainer');
  if (v) {
    v.classList.add('hidden');
    v.setAttribute('aria-hidden', 'true');
  }
  // Also close any overlay panels and backdrop
  ['searchSection', 'advancedFilters'].forEach(id => {
    const panel = document.getElementById(id);
    const btn = document.getElementById(getToggleForPanelId(id));
  ...
  // Sync view mode buttons with currentViewMode to ensure active state
...
/**
 * Apply saved UI preferences (compact cards etc.)
 */
function applySavedPreferences() {
  const compact = localStorage.getItem('compactCards');
  if (compact === 'true') {
...
function initializeDOMElements() {
  ...
  // Panels
  window.searchSection = document.getElementById('searchSection');
  window.advancedFilters = document.getElementById('advancedFilters');
  window.viewModeContainer = document.getElementById('viewModeContainer');
  // Debug: log existence of new toggle buttons and panels
  console.log('DOM elements: searchToggle=', !!document.getElementById('searchToggle'),
...
function attachSearchListeners() {
  if (searchBar) {
    ...
    searchBar.addEventListener('keydown', (e) => {
      ...
      if (e.key === 'Enter') {
        // Save to history
        saveSearchHistory(searchBar.value.trim());
      }
    });
    searchBar.addEventListener('focus', showSearchSuggestions);
    searchBar.addEventListener('blur', () => setTimeout(hideSearchSuggestions, 200));
  }
}
...
function saveSearchHistory(q) {
  ...
  try {
    ...
    const newArr = [q, ...existing.filter(s => s !== q)].slice(0, 5);
...
function showSearchSuggestions() {
  ...
  el.innerHTML = items.map(s => `<div class="suggestion-item" data-value="${s}">${s}</div>`).join('');
  ...
  el.querySelectorAll('.suggestion-item').forEach(node => {
    node.addEventListener('click', () => {
      ...
      if (sb) { sb.value = val; handleSearch({target: {value: val}}); }
...
function attachQuickActionListeners() {
  ...
  if (searchToggle) {
    try {
      ...
      console.warn('Unable to attach search toggle listener', e);
    }
  } else {
    console.warn('No #searchToggle element found');
  }

  const filtersToggle = document.getElementById('filtersToggle');
  let filtersListenerAttached = false;
  if (filtersToggle) {
    try {
      filtersToggle.addEventListener('click', toggleFiltersPanel);
      filtersListenerAttached = true;
    } catch (e) {
      console.warn('Unable to attach filters toggle listener', e);
    }
  } else {
    console.warn('No #filtersToggle element found');
  }

  // Fallback: if for some reason we couldn't attach listeners to specific buttons
  // (for example due to dynamic DOM replacements), attach delegated handlers to document
  if (!searchListenerAttached) {
    document.addEventListener('click', (e) => {
      const t = e.target.closest('#searchToggle');
      if (t) toggleSearchPanel(e);
    });
  }
  if (!filtersListenerAttached) {
    document.addEventListener('click', (e) => {
      const t = e.target.closest('#filtersToggle');
      if (t) toggleFiltersPanel(e);
    });
  }
  // viewPanelToggle removed — no delegated handler required
  const compactToggle = document.getElementById('compactToggle');
  if (compactToggle) {
    compactToggle.addEventListener('click', (e) => {
      e.preventDefault();
      const active = document.body.classList.toggle('compact-cards');
      compactToggle.setAttribute('aria-pressed', active ? 'true' : 'false');
      localStorage.setItem('compactCards', active ? 'true' : 'false');
      showToast(`Compact cards ${active ? 'enabled' : 'disabled'}`, 'info', 1200);
    });
  }
}
...
function attachContextMenuListeners() {
  ...
  document.addEventListener('click', (e) => {
    hideContextMenu();
    // Close toggle panels if click outside them
    closePanelsOnOutsideClick(e);
  });

  // Backdrop click: hide panels
  const bd = document.getElementById('panelBackdrop');
  if (bd) {
...
/**
 * Helper to toggle panels and set aria states
 */
...
/**
 * Map panel id to toggle button id. Returns null if there is no dedicated toggle button.
 */
function getToggleForPanelId(id) {
  if (!id) return null;
  if (id === 'searchSection') return 'searchToggle';
  if (id === 'advancedFilters') return 'filtersToggle';
  // viewModeContainer is inline so it has no dedicated header toggle now
  return null;
}

function toggleSearchPanel(evt) {
  evt.stopPropagation();
  console.log('toggleSearchPanel called');
  const panel = document.getElementById('searchSection');
...
function toggleFiltersPanel(evt) {
  evt.stopPropagation();
...
// toggleViewPanel removed — view mode control is inline via `viewToggle` and view-mode buttons.

function closePanelsOnOutsideClick(e) {
  // If click is not within panel or corresponding toggle buttons, hide all panels
  const panels = ['searchSection', 'advancedFilters'];
  panels.forEach(id => {
    const panel = document.getElementById(id);
    const toggleBtn = document.getElementById(getToggleForPanelId(id));
    ...
    // If all panels hidden, hide backdrop
    const bd = document.getElementById('panelBackdrop');
    if (bd) {
      const sEl = document.getElementById('searchSection');
      const aEl = document.getElementById('advancedFilters');
      // viewModeContainer is inline, not overlay — only consider overlay panels
      const any = (sEl && !sEl.classList.contains('hidden')) || (aEl && !aEl.classList.contains('hidden'));
      if (!any) bd.classList.add('hidden');
    }
  });
}
...
function attachFormListeners() {
  ...
  if (clearSearchBtn) clearSearchBtn.addEventListener('click', (e) => {
    e.preventDefault();
...
function openQuickAddModal() {
  ...
  if (quickAddModalEl) quickAddModalEl.classList.add('active');
}
...
function handleKeyboardShortcuts(e) {
  if (e.ctrlKey || e.metaKey) {
    switch (e.key) {
      case 'k':
        e.preventDefault();
        if (searchBar) {
          // Ensure search panel is visible on Ctrl+K
          const panel = document.getElementById('searchSection');
          const btn = document.getElementById('searchToggle');
          if (panel && panel.classList.contains('hidden')) {
            togglePanel(panel, btn);
  ...
  if (e.key === 'Escape') {
    ...
    closeModals();
    hideContextMenu();
    // Hide toggle panels if they are visible
    ['searchSection','advancedFilters'].forEach(id => {
      const panel = document.getElementById(id);
      const btn = document.getElementById(getToggleForPanelId(id));
      if (panel && !panel.classList.contains('hidden')) {
  ...
  if (imageViewerEl && imageViewerEl.classList.contains('active')) {
    switch (e.key) {
      case 'ArrowLeft':
        showPreviousImage();
        break;
...
function addWebsiteField() {
  ...
  <div class="website-link" data-id="${websiteId}">
    <div class="website-link-header">
      <h4>Website Link</h4>
      <div class="website-link-controls">
    ...
    <div class="form-row">
      ...
      <div class="form-group" style="flex: 1;">
        ...
        <input type="url" name="websitePicture[]" placeholder="https://image-url.jpg" onchange="updateWebsiteControlsOnPictureChange(this)">
        <small>URL for link preview image${editingSlug ? ' (add to enable ordering)' : ''}</small>
      </div>
      <div class="form-group" style="flex: 1;">
        <label>Type</label>
...
/**
 * Move a website up or down in the websites array and update the form display
 * @param {number} index - Current index of the website in the array
 * @param {string} direction - 'up' or 'down'
 */
function moveWebsite(index, direction) {
  // Get current editing actress from the actresses array
  if (!editingSlug) {
    showToast('Error: No actress is being edited', 'error');
    return;
  }
  
  const actress = actresses.find(a => a.slug === editingSlug);
  if (!actress || !actress.websites) {
    showToast('Error: Could not find websites to reorder', 'error');
    return;
  }
  
  // Make sure index is within bounds
  if (!Number.isInteger(index) || index < 0 || index >= actress.websites.length) {
    console.warn('Invalid index:', index, 'Total websites:', actress.websites.length);
    showToast('Error: Invalid website index', 'error');
    return;
  }
  ...
  if (direction === 'up' && index > 0) {
    targetIndex = index - 1;
  } else if (direction === 'down' && index < actress.websites.length - 1) {
    targetIndex = index + 1;
  } else {
  ...
  // Swap the websites in the array
  [actress.websites[index], actress.websites[targetIndex]] = 
  [actress.websites[targetIndex], actress.websites[index]];
  
  // Re-populate the form to reflect the new order
  populateFormForEditing(actress);
  
  console.log(`Website moved ${direction}: old index ${index}, new index ${targetIndex}`);
}


/**
 * Update website controls (move buttons) when picture URL is added/removed
 * @param {HTMLInputElement} inputElement - The picture URL input element
 */
function updateWebsiteControlsOnPictureChange(inputElement) {
  const hasPictureValue = inputElement.value && inputElement.value.trim().length > 0;
  const websiteElement = inputElement.closest('.website-link');
  
  if (!websiteElement) return;
  
  // Get or create the controls container
  let controlsContainer = websiteElement.querySelector('.website-link-controls');
  if (!controlsContainer) {
    const header = websiteElement.querySelector('.website-link-header');
    if (header) {
      controlsContainer = document.createElement('div');
      controlsContainer.className = 'website-link-controls';
  ...
  // Find the index of this website in the form
  const allWebsiteElements = document.querySelectorAll('#websiteLinks .website-link');
  let currentIndex = -1;
  for (let i = 0; i < allWebsiteElements.length; i++) {
    if (allWebsiteElements[i] === websiteElement) {
      currentIndex = i;
      break;
    }
  }
  
  if (currentIndex === -1) return; // Couldn't find index
  
  // Check if move buttons already exist
  const moveButtons = controlsContainer.querySelector('.btn-move-up');
  
  if (hasPictureValue && !moveButtons) {
    // Add move buttons
    ...
    moveUpBtn.onclick = () => moveWebsite(currentIndex, 'up');
    ...
    moveDownBtn.onclick = () => moveWebsite(currentIndex, 'down');
    
    // Insert before the remove button
    const removeBtn = controlsContainer.querySelector('.remove-website');
    if (removeBtn) {
      removeBtn.parentNode.insertBefore(moveDownBtn, removeBtn);
      removeBtn.parentNode.insertBefore(moveUpBtn, removeBtn);
    } else {
      controlsContainer.appendChild(moveUpBtn);
      controlsContainer.appendChild(moveDownBtn);
    }
  } else if (!hasPictureValue && moveButtons) {
    // Remove move buttons
    const moveUpBtn = controlsContainer.querySelector('.btn-move-up');
    const moveDownBtn = controlsContainer.querySelector('.btn-move-down');
    if (moveUpBtn) moveUpBtn.remove();
    if (moveDownBtn) moveDownBtn.remove();
  }
}
...
function openAdminPanel() {
  ...
  // Hide any opened panels when admin panel opens
  ['searchSection','advancedFilters','viewModeContainer'].forEach(id => {
    const panel = document.getElementById(id);
    const btn = document.getElementById(getToggleForPanelId(id));
    if (panel && !panel.classList.contains('hidden')) {
      panel.classList.add('hidden');
    ...
    <div class="manage-item ${selectedActresses.has(actress.slug) ? 'selected' : ''}">
      <div class="manage-item-info">
        <input type="checkbox" class="manage-item-checkbox" ${selectedActresses.has(actress.slug) ? 'checked' : ''} 
           ...
           <img src="${actress.thumb}" alt="${actress.name}" class="manage-item-thumb" loading="lazy" decoding="async"
    ...
    if (websiteLinks) {
      ...
      if (actress.websites && actress.websites.length > 0) {
        actress.websites.forEach((website, index) => {
          ...
          const hasImageUrl = website.picture && website.picture.trim().length > 0;
          const websiteHtml = `
            <div class="website-link" data-id="${websiteId}" data-index="${index}" data-has-picture="${hasImageUrl ? 'true' : 'false'}">
              <div class="website-link-header">
                <h4>Website Link</h4>
                <div class="website-link-controls">
                  ${hasImageUrl ? `
                  <button type="button" class="btn-move-up" onclick="moveWebsite(${index}, 'up')" title="Move up">
                    <i class="fas fa-arrow-up"></i>
                  </button>
                  <button type="button" class="btn-move-down" onclick="moveWebsite(${index}, 'down')" title="Move down">
              ...
              <div class="form-row">
                ...
                <div class="form-group" style="flex: 1;">
                  <label>Picture URL (Optional)</label>
                  <input type="url" name="websitePicture[]" value="${website.picture || ''}" placeholder="https://image-url.jpg" onchange="updateWebsiteControlsOnPictureChange(this)">
                  <small>URL for link preview image (add to enable ordering)</small>
                </div>
                <div class="form-group" style="flex: 1;">
                  <label>Type</label>
                  <select name="websiteType[]">
...
function displayActresses(list) {
  ...
  list.forEach(act => {
    ...
    card.innerHTML = `
      ...
      ` : ''}
      <img class="thumb" src="${act.thumb}" alt="${act.name} thumbnail" loading="lazy" decoding="async"
    ...
    overlay.innerHTML = `
      <button class="overlay-btn view-btn" data-slug="${act.slug}" title="View"><i class="fas fa-eye"></i></button>
      <button class="overlay-btn fav-btn" data-slug="${act.slug}" title="Favorite"><i class="fas fa-star"></i></button>
    `;
    card.appendChild(overlay);
    const viewBtn = overlay.querySelector('.view-btn');
    if (viewBtn) viewBtn.addEventListener('click', (e) => { e.stopPropagation(); openGallery(act); });
    const favBtn = overlay.querySelector('.fav-btn');
    if (favBtn) favBtn.addEventListener('click', (e) => { e.stopPropagation(); toggleFavorite(favBtn.getAttribute('data-slug')); });
  ...
  // lazy load for gallery images and inline thumbnails already handled, done.
}
...
function openGallery(actress) {
  ...
  if (backBtn) {
    ...
    showToast('Error opening gallery. Check console for details', 'error');
  }
}
...
function goBackToList() {
  ...
  app.innerHTML = `
    <div id="viewModeContainer" class="view-mode-container hidden" aria-hidden="true">
...
/**
 * attachQuickActionListeners consolidated earlier - this duplicate definition was removed
 * to ensure the panel toggle handlers (search, filters, view panel) are attached properly.
 */
